...
ADD COLUMN IF NOT EXISTS taxa_juros_diaria_atraso DECIMAL(8, 6) DEFAULT 0.033333 CHECK (taxa_juros_diaria_atraso >= 0),
ADD COLUMN IF NOT EXISTS juros_diarios_calculados DECIMAL(10, 2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS dias_atraso INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS data_ultimo_calculo TIMESTAMPTZ DEFAULT NOW();
...
CREATE INDEX IF NOT EXISTS idx_configuracoes_juros_cliente_id ON public.configuracoes_juros(cliente_id);
CREATE INDEX IF NOT EXISTS idx_configuracoes_juros_user_id ON public.configuracoes_juros(user_id);
CREATE INDEX IF NOT EXISTS idx_emprestimos_dias_atraso ON public.emprestimos(dias_atraso);
CREATE INDEX IF NOT EXISTS idx_emprestimos_data_ultimo_calculo ON public.emprestimos(data_ultimo_calculo);
